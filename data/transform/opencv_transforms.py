...
class GaussianBlur(ImageFilter.Filter):
    ...
    def __call__(self,sample):
        if np.random.uniform()<self.p:
            return F.gaussianblur(sample,self.radiu)
        else:
            return sample
...
class Shift_Padding(object):
    ...
    def __call__(self,sample):
        if np.random.random() < self.p:
            return F.shift_padding(sample,hor_shift_ratio=self.hor_shift_ratio,ver_shift_ratio=self.ver_shift_ratio,pad=self.pad)
        else:
            return sample
    def __repr__(self):
...
class Rescale(object):
    ...
    def __call__(self,sample):
        return F.rescale(sample,self.output_size)
